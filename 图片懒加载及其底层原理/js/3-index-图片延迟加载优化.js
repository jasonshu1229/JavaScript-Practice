...
let imageModule = (function () {
  ...
  function lazyFunc() {
    !lazyImageBoxes
      ? (lazyImageBoxes = Array.from(
          document.querySelectorAll(".lazyImageBox")
        ))
      : null;
    ...
    lazyImageBoxes.forEach((lazyImageBox) => {
      ...
      // todo: 加载条件：
      // todo:  情况1：（图片底部未全出来时，就已经加载完毕）待加载的图片盒子顶部距离body的距离(offsetTop) <= 浏览器的可视区域(clientHeight) + 滚动条滚过的距离(scrollTop)
      // todo:  情况2：（图片底部全出来之后，才延迟加载图片）待加载的图片盒子顶部距离body的距离(offsetTop) <= 浏览器的可视区域(clientHeight) + 滚动条滚过的距离(scrollTop)
      
      // 此时使用的情况2，为了更好地看出延迟加载的效果
      let notLoadingBox = utils.offset(lazyImageBox).top + lazyImageBox.offsetHeight;
      let clientScrollHeight = winHeight + document.documentElement.scrollTop;
      if (notLoadingBox <= clientScrollHeight) {
